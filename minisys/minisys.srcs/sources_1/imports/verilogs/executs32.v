...
always @* begin  // 6种移位指令
       if(Sftmd)
        case(Sftm[2:0])
            ...
            3'b011:Sinput = $signed(Binput) >>> Shamt;         	   //sra rd,rt,shamt 000011
            3'b111:Sinput = $signed(Binput) >>> Ainput[4:0];		   //srav rd,rt,rs 000111
    ...
    always @* begin
        if(((ALU_ctl==3'b111) && (Exe_code[3]==1))||((ALU_ctl[2:1]==2'b11) && (I_format==1))) //slti(sub)  处理所有SLT类的问题
            ALU_Result = (Ainput < Binput) ? 32'h00000001 : 32'b0;//?????有符号无符号
