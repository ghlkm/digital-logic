...
module minisys(rst,clk,led2N4,switch2N4);
    input rst;               //疑问????Reset??????????λ
    input clk;               //疑问????100MHz??????
    input[23:0] switch2N4;    //疑问?????????
    output[23:0] led2N4;      //led????????????
    
    wire clock;              //clock: ?????????????
    wire iowrite,ioread;     //I/O??д???
    wire[31:0] write_data;   //дRAM??IO??????
    wire[31:0] rdata;        //疑问?RAM??IO??????
    wire[15:0] ioread_data;  //疑问?IO??????
    wire[31:0] pc_plus_4;    //PC+4
    wire[31:0] read_data_1;  //
    wire[31:0] read_data_2;  //
    wire[31:0] sign_extend;  //疑问??????
    wire[31:0] add_result;   //
    wire[31:0] alu_result;   //
    wire[31:0] read_data;    //RAM?ж????????
    wire[31:0] address;
    wire alusrc;
    wire branch;
